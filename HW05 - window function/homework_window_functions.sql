...
FROM 
	...
	JOIN MonthAmountCumulative mac ON mac.endm = EOMONTH(Invoices.InvoiceDate)
;
...
--   Время ЦП = 141 мс, затраченное время = 309 мс.

-- Для запроса ниже
-- Время работы SQL Server:
--   Время ЦП = 47 мс, затраченное время = 241 мс.
...
-- Уточнение: если в один день клиент более одного раза покупал один и тот же из двух самых дорогих товаров, то выводим только одну запись
