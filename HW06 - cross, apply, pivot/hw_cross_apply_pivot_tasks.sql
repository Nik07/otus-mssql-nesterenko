...
-- в задание не сказано, что нужно упорядочить по месцу, поэтому можно и так 
WITH CTE AS (
	SELECT 
		[PartName] = SUBSTRING(CustomerName, CHARINDEX('(', CustomerName) + 1, LEN(CustomerName) - CHARINDEX('(', CustomerName) - 1),
		[InvMonth] = FORMAT(Invoices.InvoiceDate, '01.MM.yyyy'),
		InvoiceID
	FROM 
		Sales.Invoices
		JOIN Sales.Customers ON Invoices.CustomerID = Customers.CustomerID
	WHERE Customers.CustomerID BETWEEN 2 AND 6)
SELECT 
	InvMonth, [Peeples Valley, AZ], [Medicine Lodge, KS], [Gasport, NY], [Sylvanite, MT], [Jessie, ND]
FROM CTE
PIVOT (COUNT(InvoiceID) FOR PartName IN (
	[Peeples Valley, AZ], 
	[Medicine Lodge, KS], 
...
-- если для клиента из самых дорогих товаров несколько, то будет выведена информация только по двум (случайно)
-- если для клиента самый дорогой один товар и на втором месте (из самых дорогих товаров) несколько с одинаковой ценой,
-- то будет выведена информация по первому месту и только по одному товару (случайно) из второго места (--WITH TIES)
